...
path = F"/Users/janjacob/Downloads/Step 5: Data Wrangling and Exploration/{season}_bs_by_date"
...
select_date = "20111219"
year = select_date[0:4]
month = select_date[4:6]
day = select_date[6:]

with open(F"{path}/{select_date}_bs.csv", "a") as f:
    # Get all games on that day using CBS
    url_cbs = F'https://www.cbssports.com/college-basketball/scoreboard/FBS/{select_date}/'
    while True:
        try:
            ...
            print(
                F"Ecountered error requesting CBS url. Waited {r1} seconds till next request...")
            time.sleep(r1)
    ...
    for j in range(0, len(pts_page), 2):
    ...
    print(F"Running this date: {select_date}")
    print(F"We have {len(matchups)} matchups this day")

    lines = [line.rstrip() for line in open(F"{path}/{season}_tracker.txt")]
    b = True
    for k in lines:
        if select_date not in k:
            continue
        else:
            b = False
            break
    ...
    if b:
        with open(F"{path}/{season}_tracker.txt", "a") as track:
            track.write(
                F"{select_date} : {len(matchups)} played this day \n")

    if len(cells) == 0:  # No games happened
        f.write("No games were played on this date.")
        sys.exit("No games were played on this date.")

    for i in range(len(pts_page)):
        print(F"Matchup {i}: {matchups[i]}")
        # Use ESPN to get our data
        query = F"{matchups[i][0]} vs {matchups[i][1]} mens college basketball espn {year}/{month}/{day}"

        while True:
            try:
                for website in search(query, tld="co.in", num=5, stop=5, pause=10):
                    if 'espn' in website:
                        espn_url = website
                        break
                break
            except:
                r2 = random.randint(1400, 1600)
                print(
                    F"Encountered error requesting Google url. Waiting {r2} seconds till next request...")
                time.sleep(r2)
        ...
        while True:
            try:
                ...
                r3 = random.randint(90, 210)
                print(
                    F"Encountered error requesting ESPN url. Waiting {r3} seconds till next request...")
                time.sleep(r3)
